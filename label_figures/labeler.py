...
def present_images(img_dir, dataset, conn, cur):
  ...
  for img_file in imgs[0:5]:
    ...
    cur.execute("INSERT INTO img_labels VALUES('{0}', '{1}', {2}, {3}, {4}, {5})".format(img_file, dataset, has_img, num_plots, has_color, has_diag))
    conn.commit()
...
def get_db_cursor():
  ...
  cur.execute("""CREATE TABLE IF NOT EXISTS img_labels (file_name text, dataset text, has_image integer, num_graphics integer, 
                has_color integer, has_diagram integer) """)
  return conn, cur

def row_exists(cur, path):
  cur.execute("SELECT * FROM img_labels WHERE file_path = '" + path + "';")
